...
from app.schemas import RatingEnum, StateEnum
...
class FSRSManager:
    ...
    def review_card(self, user_word: UserWord, rating: RatingEnum,
    ...
    def get_card_retrievability(self, user_word: UserWord) -> float:
        """Get the current retrievability (probability of recall) for a card"""
        card = Card.from_dict(user_word.fsrs_card_data)
        return self.scheduler.get_card_retrievability(card)

    def is_card_due(self, user_word: UserWord) -> bool:
        """Check if a card is due for review"""
        card = Card.from_dict(user_word.fsrs_card_data)
        now = datetime.now(timezone.utc)
        return card.due <= now

    def get_next_review_time(self, user_word: UserWord) -> Optional[datetime]:
        """Get the next review time for a card"""
        card = Card.from_dict(user_word.fsrs_card_data)
        return card.due

    def get_card_state(self, user_word: UserWord) -> StateEnum:
        """Get the current state of a card"""
        card = Card.from_dict(user_word.fsrs_card_data)
        return StateEnum(card.state)

    def reschedule_card(self, user_word: UserWord, review_logs: list[ReviewLog]) -> Dict[str, Any]:
...
class WordLearningService:
    ...
    def create_user_word(self, user_id: int, word_id: int) -> UserWord:
    ...
    def review_word(self, user_word: UserWord, rating: RatingEnum,
                    response_time_seconds: Optional[float] = None,
                    lesson_context: Optional[int] = None) -> tuple[UserWord, Review]:
        ...
        return user_word, review

    def get_words_due_for_review(self, user_id: int, limit: int = 20) -> list[UserWord]:
        ...
        for user_word in user_words:
            # maybe problem here 🔴:🔴:🔴
            if self.fsrs_manager.is_card_due(user_word):
                due_words.append(user_word)
    ...
    def get_word_progress(self, user_word: UserWord) -> Dict[str, Any]:
